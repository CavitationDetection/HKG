'''
Description: Representation learning (Vision Transformer)+ Graph Convolutional Network (GCN)
Author: Yu Sha
...
vit_h_14:   h-->huge model(depth: 32 Transformer layer, Embedding size:1280, Multi-head self-attention: 16, Feedforward size: 5120)
            14: patch size
"""
# representation learning is ViT (Vision Transformer)
def vit_b_16_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
    model = models.vit_b_16(pretrained = pretrained)
    return ViT_ADD_GCN(model, num_classes, tau = tau, eta = eta, adj_file = adj_file, in_channel = in_channel)

def vit_b_32_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
    model = models.vit_b_32(pretrained = pretrained)
    return ViT_ADD_GCN(model, num_classes, tau = tau, eta = eta, adj_file = adj_file, in_channel = in_channel)

def vit_l_16_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
    model = models.vit_l_16(pretrained = pretrained)
    return ViT_ADD_GCN(model, num_classes, tau = tau, eta = eta, adj_file = adj_file, in_channel = in_channel)

def vit_l_32_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
    model = models.vit_l_32(pretrained = pretrained)
    return ViT_ADD_GCN(model, num_classes, tau = tau, eta = eta, adj_file = adj_file, in_channel = in_channel)

def vit_h_14_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
