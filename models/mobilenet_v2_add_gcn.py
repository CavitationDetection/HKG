'''
Description: Representation learning (MobileNet_v2) + Graph Convolutional Network (GCN)
Author: Yu Sha
...
class MobileNet_v2_ADD_GCN(nn.Module):
    ...
    def __init__(self, model, num_classes, in_channel = 300, tau = 0, eta = 0, adj_file = None):
        super(MobileNet_v2_ADD_GCN, self).__init__()
...
# representation learning is MobileNet_v2
def mobilenet_v2_add_gcn(num_classes, tau, eta, pretrained = False, adj_file = None, in_channel = 300):
    model = models.mobilenet_v2(pretrained = pretrained)
    return MobileNet_v2_ADD_GCN(model, num_classes, tau = tau, eta = eta, adj_file = adj_file, in_channel = in_channel)
